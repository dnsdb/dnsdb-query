...
DEFAULT_CONFIG_FILES = filter(os.path.isfile,
                              ('/etc/dnsdb-query.conf',
                               os.path.expanduser('~/.dnsdb-query.conf')))
...
class DnsdbClient(object):
    def __init__(self, server, apikey, limit=None, http_proxy=None,
                 https_proxy=None):
        self.server = server
        self.apikey = apikey
        self.limit = limit
    ...
    def query_rrset(self, oname, rrtype=None, bailiwick=None, before=None,
                    after=None):
        if bailiwick:
            if not rrtype:
                rrtype = 'ANY'
...
def main():
    ...
    parser = optparse.OptionParser(epilog='Time formats are: "%Y-%m-%d", '
                                          '"%Y-%m-%d %H:%M:%S", "%d" '
                                          '(UNIX timestamp), "-%d" '
                                          '(Relative time in seconds), '
                                          'BIND format '
                                          '(e.g. 1w1h, (w)eek, (d)ay, (h)our, '
                                          '(m)inute, (s)econd)')
    ...
    parser.add_option('', '--before', dest='before', type='string',
                      help='only output results seen before this time')
    parser.add_option('', '--after', dest='after', type='string',
    ...
    try:
        ...
        logger.error('Could not parse before: {}'.format(options.before))

    try:
        if options.after:
            options.after = time_parse(options.after)
    except ValueError:
        logger.error('Could not parse after: {}'.format(options.after))
        ...
        if options.sort:
            ...
            if len(results) > 0:
                if not options.sort in results[0]:
                    ...
                    sort_keys.sort()
                    logger.error('dnsdb_query: invalid sort key "%s". valid sort keys are %s\n' % (options.sort, ', '.join(sort_keys)))
                    sys.exit(1)
